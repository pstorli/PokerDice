...
import com.pstorli.pokerdice.util.Consts.WON_NAME
...
class PokerPrefs (var prefs: Prefs) : PokerDS {
    ...
    override suspend fun loadDAO (what: Array<Persist>): PokerDAO
    {
        ...
        for (which in what) {
            when (which) {
                ...
                Persist.WON                 -> pokerDAO.won             = prefs.getInt        (WON_NAME)
            }
        }
        return pokerDAO
    }
    ...
    override suspend fun saveDAO (pokerDAO: PokerDAO, what: Array<Persist>)
    {
        ...
        for (which in what) {
            when (which) {
                ...
                Persist.WON               -> prefs.putInt         (WON_NAME,                            pokerDAO.won)
            }
        }
    }
     ...
     */
    fun loadBoard (): Array<Die> {
        // Create an empty board, then fill it with values.
        val board = Array(BOARD_SIZE*BOARD_SIZE) { Die() }

        // Loop through the rows and cols.
        for (index in 0 until BOARD_SIZE*BOARD_SIZE) {
            // Get saved pref board row/col value and assign to board.
            board[index] = Die (prefs.getInt (squareName(index)))
        }
     ...
     */
    fun saveBoard (board: Array<Die>) {
        prefs.putBool(Consts.GAME_SAVED, true)

        // Loop through the rows and cols.
        for (index in 0 until BOARD_SIZE*BOARD_SIZE) {
            prefs.putInt (squareName(index), board.get(index).num)
        }
    }
}