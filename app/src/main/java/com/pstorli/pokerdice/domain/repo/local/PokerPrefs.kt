...
class PokerPrefs (var prefs: Prefs) : PokerDS {
    ...
    fun resetBoard () {
        ...
        for (row in 0 until BOARD_SIZE_VAL) {
            for (col in 0 until BOARD_SIZE_VAL) {
                prefs.putInt (squareName(row,col), EMPTY_SQUARE_VAL)
            }
        }
    }
    ...
    fun loadBoard (): Array<IntArray> {
        ...
        for (row in 0 until BOARD_SIZE_VAL) {
            for (col in 0 until BOARD_SIZE_VAL) {
                ...
                board[row][col] = prefs.getInt (squareName(row,col))
            }
        }
     ...
     */
    fun saveBoard (board: Array<IntArray>) {
        prefs.putBool(Consts.GAME_SAVED, true)

        // Loop through the rows and cols.
        for (row in 0 until BOARD_SIZE_VAL) {
            for (col in 0 until BOARD_SIZE_VAL) {
                prefs.putInt (squareName(row,col), board[row][col])
            }
        }
    }
}